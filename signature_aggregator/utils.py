...
class Wrappers:
    @staticmethod
    async def sign(function: types.FunctionType, dkg_key, destination_address: Dict[str, str], destination_peer_id: PeerID, protocol_id: TProtocol,
                   message: Dict, result: Dict = None, timeout: float = 5.0, semaphore: trio.Semaphore = None):
        await function(destination_address, destination_peer_id, protocol_id,
                                      message, result,timeout, semaphore)
        if protocol_id == PROTOCOLS_ID['sign'] and result is not None and\
                            result.get(destination_peer_id) is not None and \
                            result[destination_peer_id]['status'] == 'SUCCESSFUL':
